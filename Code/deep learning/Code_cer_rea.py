...
def mape(pred, actual):
    error = np.subtract(pred, actual)
    sqerror= np.sum(np.abs(error)/actual)/actual.shape[0]
    return sqerror
# In[3]:
...
def Plot_Mean(name, Overall_Analysis, n_steps_out):
    ...
    Plot(name, labels, width, MLP_Adam, MLP_Sgd, LSTM, Bi_LSTM, EN_DC, RNN, CNN, yer1, yer2, yer3, yer4, yer5, yer6,
         yer7, "", "", "Train&Test_RMSE_Mean_Comparison", 4)
...
def Plot_Step_RMSE_Mean(name, Overall_Analysis, n_steps_out):
    ...
    width = 0.12  # the width of the bars
    Plot(name, labels, width, MLP_Adam, MLP_Sgd, LSTM, Bi_LSTM, EN_DC, RNN, CNN, yer1, yer2, yer3, yer4, yer5, yer6,
         yer7, "Steps", "RMSE(Mean)", "Step_RMSE_Comparison", 2)


def Plot(name, labels, width, MLP_Adam, MLP_Sgd, LSTM, Bi_LSTM, EN_DC, RNN, CNN, yer1, yer2, yer3, yer4, yer5, yer6,
         yer7, xlabel, ylabel, Gname, cap):
    ...
    fig, ax = plt.subplots()
    rects1 = ax.bar(r1, MLP_Adam, width, edgecolor='black', yerr=yer1, capsize=cap, label='MLP-Adam')
    rects2 = ax.bar(r2, MLP_Sgd, width, edgecolor='black', yerr=yer2, capsize=cap, label='MLP-SGD')
    ...
    plt.xticks([r + width for r in range(len(MLP_Adam))], labels, fontsize=18)
...
def MODEL_MLP_adam(x_train, x_test, y_train, y_test, Num_Exp, n_steps_in, n_steps_out, Epochs, Hidden):
    train_acc = np.zeros(Num_Exp)
    test_acc = np.zeros(Num_Exp)
    Step_RMSE = np.zeros([Num_Exp, n_steps_out])
    mlp_adam = MLPRegressor(hidden_layer_sizes=(Hidden,), activation='relu', solver='adam', alpha=0.1, max_iter=Epochs,
                            tol=0)
...
def MODEL_MLP_sgd(x_train, x_test, y_train, y_test, Num_Exp, n_steps_in, n_steps_out, Epochs, Hidden):
    train_acc = np.zeros(Num_Exp)
    test_acc = np.zeros(Num_Exp)
    Step_RMSE = np.zeros([Num_Exp, n_steps_out])
    mlp_sgd = MLPRegressor(hidden_layer_sizes=(Hidden,), activation='relu', solver='sgd', alpha=0.1, max_iter=Epochs,
                           tol=0)
    Best_RMSE = 1000  # Assigning a large number
...
def main():
    ...
    for i in range(1, 6):
        ...
        problem = i
        if problem == 1:
            TrainData = pd.read_csv("rea1_cer_train.csv", index_col=0)
            TrainData = TrainData.values
            TestData = pd.read_csv("rea1_cer_test.csv", index_col=0)
            TestData = TestData.values
            name = "rea1"
        if problem == 2:
            TrainData = pd.read_csv("rea3_cer_train.csv", index_col=0)
            TrainData = TrainData.values
            TestData = pd.read_csv("rea3_cer_test.csv", index_col=0)
            TestData = TestData.values
            name = "rea3"
        if problem == 3:
            TrainData = pd.read_csv("rea5_cer_train.csv", index_col=0)
            TrainData = TrainData.values
            TestData = pd.read_csv("rea5_cer_test.csv", index_col=0)
            TestData = TestData.values
            name = "rea5"
        if problem == 4:
            TrainData = pd.read_csv("rea7_cer_train.csv", index_col=0)
            TrainData = TrainData.values
            TestData = pd.read_csv("rea7_cer_test.csv", index_col=0)
            TestData = TestData.values
            name = "rea7"
        if problem == 5:
            TrainData = pd.read_csv("rea9_cer_train.csv", index_col=0)
            TrainData = TrainData.values
            TestData = pd.read_csv("rea9_cer_test.csv", index_col=0)
            TestData = TestData.values
            name = "rea9"
        ...
        for k in range(1, 5):
            ...
            method = k
            if method == 1:
                train_acc, test_acc, Step_RMSE, Best_Predict_Test = MODEL_MLP_adam(x_train, x_test, y_train, y_test,
                                                                                   Num_Exp, n_steps_in, n_steps_out,
                                                                                   Epochs, Hidden)
                Mname = "MODEL_MLP_adam"

            if method == 2:
                train_acc, test_acc, Step_RMSE, Best_Predict_Test = MODEL_LSTM(x_train, x_test, y_train, y_test,
                                                                               Num_Exp, n_steps_in, n_steps_out, Epochs,
                                                                               Hidden)
                Mname = "MODEL_LSTM"
            if method == 3:
                train_acc, test_acc, Step_RMSE, Best_Predict_Test = MODEL_Bi_LSTM(x_train, x_test, y_train, y_test,
                                                                                  Num_Exp, n_steps_in, n_steps_out,
                                                                                  Epochs, Hidden)
                Mname = "MODEL_Bi_LSTM"

            if method == 4:
                train_acc, test_acc, Step_RMSE, Best_Predict_Test = MODEL_CNN(x_train, x_test, y_train, y_test, Num_Exp,
            ...
            Best_Predict_Test = pd.DataFrame(Best_Predict_Test)  # 这种形式才能读取进表格里
            print(Best_Predict_Test)
            Best_Predict_Test.to_csv("Results/"+name+Mname+"cer.csv")
